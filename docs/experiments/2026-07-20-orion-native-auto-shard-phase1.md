...
artifact 安装和重启激活、标准 Search/Query benchmark 以及三方法 matrix 汇总入口；但
还没有完整在线 CRUD、热 generation 激活或 native 四节点正式 Recall–QPS 结果。因此
不能把旧 custom-shard/client-hint/synthetic-ID 实验表述为本架构的性能结果，也不能把
Phase 1 称为完整生产级的 Qdrant default-sharding replacement。
...
- `upper_k`、upper search EF、Dynamic EF base/factor；
...
隔离和 worker 资源配置。`manifest` 写入上面的 `$DEPLOYMENT_MANIFEST`。
...
"$PYTHON" tools/orion_native_layout.py --hdf5-path "$DATASET" --output-dir "$ORION_LAYOUT" --generation "$GENERATION" --p "$SHARDS" --vector-distance cosine --sample-denominator 32 --upper-m 32 --upper-ef-construction 100 --upper-search-ef 100 --upper-k 100 --k-overlap 10 --dynamic-ef-base 20 --dynamic-ef-factor 4 --cargo tools/cargo_in_docker.sh --cargo-target-dir "$CARGO_TARGET_DIR"
```
...
"$PYTHON" tools/simple_kmeans_native_layout.py --hdf5-path "$DATASET" --output-dir "$SIMPLE_LAYOUT" --generation "$GENERATION" --p "$SHARDS" --vector-distance cosine --nprobe 32 --lower-hnsw-ef 80 --kmeans-train-size 10000 --kmeans-iters 10 --kmeans-seed 1 --cargo tools/cargo_in_docker.sh --cargo-target-dir "$CARGO_TARGET_DIR"
```
...
"$PYTHON" tools/native_auto_shard_prepare.py --method hash_all --topology "$TOPOLOGY" --run-id "$RUN_ID" --collection "$HASH_COLLECTION" --base-url "$BASE_URL" --output-dir "$PROOF_ROOT/prepare-hash-all" --hdf5-path "$DATASET" --p "$SHARDS" --vector-distance cosine --hnsw-m 32 --ef-construct 100 --batch-size 512
```
...
"$PYTHON" tools/native_auto_shard_prepare.py --method orion --topology "$TOPOLOGY" --run-id "$RUN_ID" --collection "$ORION_COLLECTION" --base-url "$BASE_URL" --output-dir "$PROOF_ROOT/prepare-orion" --layout-dir "$ORION_LAYOUT" --hnsw-m 32 --ef-construct 100 --batch-size 512 --cargo-target-dir "$CARGO_TARGET_DIR"
```
...
"$PYTHON" tools/native_auto_shard_prepare.py --method simple_kmeans --topology "$TOPOLOGY" --run-id "$RUN_ID" --collection "$SIMPLE_COLLECTION" --base-url "$BASE_URL" --output-dir "$PROOF_ROOT/prepare-simple-kmeans" --layout-dir "$SIMPLE_LAYOUT" --hnsw-m 32 --ef-construct 100 --batch-size 512 --cargo-target-dir "$CARGO_TARGET_DIR"
```

routed import 中断时，只能在 exact bundle/checkpoint/collection 一致时对新的 proof 输出
目录使用 `--resume`。导入和 artifact 激活完成后，prepare 还会从 controller 分别发一条
不含 shard、EP、per-shard EF 或 source-ID hint 的标准 Search 与简单 Query，要求结果非空
且 external IDs 唯一。prepare 成功会写 `preparation_manifest.json`，证明 collection config、
provenance metadata、image manifest、import/install command、标准 API smoke、
logical/physical count 和最终 placement；它本身不宣称已经完成 Recall–QPS 测量。
...
`null/unknown_without_server_trace`，不会拿 artifact 参数冒充实际观测。
...
`recall_qps_points.csv`、`pareto_frontier.csv`、`same_recall_selection.csv`、共享 provenance
manifest，以及 Recall–QPS、Recall–latency、Recall–visited-shards、Recall–EF-sum 图。
它默认选择 0.90/0.95、窗口 0.003 的同召回点；原始 null routing costs 保持 null，不会被
强填。上述命令和模板是可执行入口，不是已经运行完成的正式结果。
...
4. Orion 必须保留完整 membership union、multi-assignment、ordered MultiEP、Dynamic EF
   和 external-ID dedup；禁止 adaptive shard pruning。
5. controller/client CPU 隔离、worker CPU、optimizer/indexing 状态和网络条件一致。
6. 正式计时期间四 peers healthy、无 indexing、无 optimizer backlog、无 transfer。
