...
use api::grpc::qdrant::{
    ClearPayloadPointsInternal, CoreSearchBatchByShardCompactInternal,
    CoreSearchBatchByShardInternal, CoreSearchBatchPointsInternal, CountPointsInternal,
    CountResponse, CreateFieldIndexCollectionInternal, DeleteFieldIndexCollectionInternal,
...
impl PointsInternal for PointsInternalService {
    ...
    async fn core_search_batch_by_shard_compact(
        &self,
        request: Request<CoreSearchBatchByShardCompactInternal>,
    ) -> Result<Response<SearchBatchResponse>, Status> {
        validate_and_log(request.get_ref());

        let CoreSearchBatchByShardCompactInternal {
            collection_name,
            query_templates,
            searches,
            timeout,
        ...
        let hw_data =
            self.get_request_collection_hw_usage_counter_for_internal(collection_name.clone());
        let res = core_search_batch_by_shard_compact(
            self.toc.as_ref(),
            collection_name,
            wire_version,
            query_templates,
