#!/usr/bin/env python3
"""Audit the remaining shard-24-preserving hotspot candidates."""
...
def build(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    output = root / "online-v15"
    plan_dir = root / "refine-v15-hotspot-followup"
    audit_path = output / "evidence-audit.json"
    report_path = output / "RESULTS_zh.md"
    if audit_path.exists() or report_path.exists():
        raise FileExistsError("refusing to overwrite existing online-v15 audit/report")
    summary_path = output / "screen-hotspot-followup-summary.json"
    completion_path = output / "screen-hotspot-followup-complete.json"
    ...
    prior_audit_path = root / "online-v14/evidence-audit.json"
    ...
    add(
        ...
        {"status": manifest.get("status"), "selected": manifest.get("selected_candidate_count")},
    )
    baseline_shard_24 = int(plan["placements"][BASELINE]["24"])
    for strategy, mapping in plan["placements"].items():
        add(f"{strategy}: shard 24 preserved", int(mapping["24"]) == baseline_shard_24, int(mapping["24"]))
    ...
    add(
        "all follow-up candidates lose to baseline minimum",
    ...
    add("online placement inventory", phase_count == 81 and placement_count == 36, {"phase_count": phase_count, "unique_placement_count": placement_count})
    ...
    audit = {
        ...
        "record_type": "orion_physical_placement_hotspot_followup_evidence_audit",
        "claim_boundary": (
            "the remaining prioritized shard-24-preserving hotspot candidates are negative; "
            "swap_24_21 remains best among 36 online-tested placements"
        ),
    ...
    if failed:
        raise RuntimeError(f"online-v15 evidence audit failed: {[row['name'] for row in failed]}")
    report = [
        "# swap_24_21 热点卸载后续筛选结果",
        "",
        "在 4 台物理机、32 个逻辑分片、64 个 Qdrant 核的固定合同下，四个后续热点卸载候选全部在线失败。",
        "",
        ...
        *[
        ...
        "",
        f"baseline A/B 漂移 {drift:+.3f}%；所有候选低于 baseline 最低端点 {baseline_min:.2f} QPS。",
